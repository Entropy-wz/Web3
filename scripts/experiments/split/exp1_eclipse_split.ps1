param(
    ...
    [string]$OutputRoot = "artifacts/paper_runs_split/exp1",
...
foreach ($seed in $Seeds) {
    ...
    $baseArgs = @($common + @("--output-dir", "$OutputRoot/s${seed}_baseline"))
    $atkArgs = @($common + @(
            ...
            "--output-dir", "$OutputRoot/s${seed}_attack"
        ))
    if (-not $WithPaperCharts) {
        $baseArgs += "--no-paper-charts"
        $atkArgs += "--no-paper-charts"
    }
    $runs += @{
        Name = "s${seed}_baseline"
        Args = $baseArgs
    }
    $runs += @{
        Name = "s${seed}_attack"
...
Write-Host "[INFO] exp1 part=$Part/$totalParts running indexes $startIdx..$endIdx"
for ($i = $startIdx; $i -le $endIdx; $i++) {
    Invoke-Phase5 -ArgsList $runs[$i].Args
}

Write-Host "[DONE] exp1 split part $Part completed."
