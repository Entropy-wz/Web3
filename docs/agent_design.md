...
## 14. 实例附录：24-30 人初始画像（LUNA 崩盘场景）
以下为当前代码已落地的一组可直接复现的画像实例，作为论文实验模板参考。
...
### 14.3 角色画像要点（已写入 profile）
1. 项目方（Defender）：
   - 目标：尽量维持锚定；若救市无望，优先保护协议不彻底失控。
   - 对外话术：持续安抚市场。
   - 治理倾向：前期可放宽吸压参数，后期可考虑收紧铸造。
2. 巨鲸（Opportunist）：
   - 目标：利润最大化并在系统失稳前完成撤离。
   - 对外话术：在关键时点释放 FUD。
   - 治理倾向：关注并争夺治理权，倾向阻止影响套利退出路径的提案。
3. 散户三类：
   - 恐慌型：风险承受低，看到脱锚和流言会快速卖出。
   - 跟风型：主要跟随巨鲸和项目方叙事切换立场。
   - 信仰型：倾向“越跌越买”，直到触发极端止损条件。

### 14.4 初始资金模板（当前实现）
1. 项目方：UST 10M / USDC 100M / LUNA 10M。
2. 巨鲸 A：UST 50M / USDC 20M / LUNA 0。
3. 巨鲸 B：UST 20M / USDC 50M / LUNA 0。
4. 恐慌型散户：UST 10K / USDC 0 / LUNA 0。
5. 跟风型散户：UST 8K / USDC 1K / LUNA 1K。
6. 信仰型散户：UST 5K / USDC 2K / LUNA 3K。

### 14.5 黑天鹅触发脚本（Tick 0）
本实例默认在 Tick 0 注入两条冲击动作：
1. 巨鲸 A 发起大额抛售（Pool_A, 卖出 30M UST）。
2. 巨鲸 A 发送恐慌发言：
   - “UST is a Ponzi, it's over. Getting out now.”

说明：可通过运行参数关闭该默认冲击，用于做对照组实验。

### 14.6 本实例的研究价值
1. 能稳定制造“价格冲击 -> 叙事扩散 -> 散户踩踏 -> 治理争夺”的连续链条。
2. 能清晰对比“理性强势角色”和“有限理性散户”在同一市场中的行为差异。
3. 便于论文中复现实验与展示关键图表（脱锚、拥堵、治理集中度、恐慌传播）。

### 14.7 适用范围与边界（避免误读）
1. 本附录描述的是“LUNA 崩盘实验模板”的专用画像，不等同于系统全部场景默认配置。
2. 若使用 `build_luna_crash_bootstrap` 与对应可视化脚本，本附录中的人数、资金、黑天鹅触发与角色策略即为实际执行口径。
3. 若改用其他初始化入口（例如通用默认画像或自定义配置文件），应以当次运行配置为准，并建议在论文附录记录“画像版本号 + 运行参数”。
